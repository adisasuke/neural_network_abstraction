...
if __name__ == "__main__" :
        ...
        model, inp, means, stds, ls_obj = read_tensorflow_net(netname, image_size, True)#N why its not taking perturbation(assuming on its own?)
        ...
        # well, here modl is containging all the continuous epsilon
        # print(eta_set)
        # print(20*'*')
        ...
        # model_string = str(modl)
        # print(model_string)

        # sample = open('first_eta.txt','w')
        # print (sorted ([(d, modl[d]) for d in modl], key = lambda x: str(x[0])),file=sample)
        # sample.close()

        # print (len(sorted ([(d, modl[d]) for d in modl], key = lambda x: str(x[0]))))
        model_list = sorted ([(d, modl[d]) for d in modl], key = lambda x: str(x[0]))
        model_string=model_list
        # var formattedString = model_string.split(",").join("\n")

        for idx,val in model_list:
        ...
        """ i dont know how but this prints
        our modl completely maybe because its converted into a list""" 
...
#########################################################################################################################
        ...
        with tf.Session() as sess:
                pred1 = sess.run(model, feed_dict={inp: newimage})
                newpred1 = pred1.flatten()
                        
        print("label of modified image classified by model:"  + str(np.argmax(newpred1)))
